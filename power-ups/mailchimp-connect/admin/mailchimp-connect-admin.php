...
//=============================================
class WPMailChimpConnectAdmin extends WPLeadOutAdmin {
    ...
    function leadout_build_esp_settings_page ()
    {
        ...
        add_settings_section($this->power_up_settings_section, $this->power_up_icon . "MailChimp", '', LEADOUT_ADMIN_PATH);
        add_settings_field('li_mls_api_key', 'API key', array($this, 'li_mls_api_key_callback'), LEADOUT_ADMIN_PATH, $this->power_up_settings_section);
    ...
    function li_print_synced_lists ()
    {
        ...
        if ( isset($li_mls_api_key ) )
        {
            ...
            if ( ! $synced_list_count ) {
                echo '<p>MailChimp connected succesfully! <a href="' . get_bloginfo('wpurl') . '/wp-admin/admin.php?page=leadout_tags">Select a tag to send contacts to MailChimp</a>.</p>';
            } else {
                echo '<p><a href="' . get_bloginfo('wpurl') . '/wp-admin/admin.php?page=leadout_tags">Edit your tags</a> or <a href="http://admin.mailchimp.com/lists/new-list/" target="_blank">Create a new list on MailChimp.com</a></p>';
            }
        }
    }
