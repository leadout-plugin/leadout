...
jQuery(document).ready( function ( $ ) {
	...
	$("#filter_content").select2({
	    query: function( query ) {
	    	...
	    	$.ajax({
				...
				data: {
					"action": "leadout_get_posts_and_pages", 
					"search_term": key
				},
	...
	$("#filter_form").select2({
	    query: function( query ) {
	    	...
	    	$.ajax({
				...
				url: li_admin_ajax.ajax_url,
				data: {
					"action": "leadout_get_form_selectors", 
					"search_term": key
				},
				success: function(data){
					// Force override the current tracking with the merged value
	...
	$('#leadout-contact-status').change( function ( e ) {
		$('#leadout-contact-status-button').addClass('button-primary');
	});

	$('input[name=popup-position]:radio').change( function ( e ) {
		$('#btn-activate-subscribe').attr('href', window.location.href + '&leadout_action=activate&power_up=subscribe_widget&redirect_to=' + encodeURIComponent(window.location.href + '&activate_popup=true&popup_position=' + $("input:radio[name='popup-position']:checked").val()));
	});

	$('input[name=esp]:radio').change( function ( e ) {
		$('#btn-activate-esp').attr('href', window.location.href + '&leadout_action=activate&power_up=' + $("input:radio[name='esp']:checked").val() + '&redirect_to=' + encodeURIComponent(window.location.href + '&activate_esp=true'));
	});
...
jQuery(document).ready( function ( $ ) {
	...
	$('#leadout-contacts input:checkbox').not('thead input:checkbox, tfoot input:checkbox').bind('change', function ( e  ){
		var cb_count = 0;
		var selected_vals = '';
		var $btn_selected = $('#leadout-export-selected-leads');
		
		var $input_selected_vals = $('.leadout-selected-contacts');
		var $cb_selected = $('#leadout-contacts input:checkbox:checked').not('thead input:checkbox, tfoot input:checkbox');
	...
	$('#cb-select-all-1, #cb-select-all-2').bind('change', function ( e ) { 
		...
		var $this = $(this);
		var $btn_selected = $('#leadout-export-selected-leads');
		var $cb_selected = $('#leadout-contacts input:checkbox').not('thead input:checkbox, tfoot input:checkbox');
		var $input_selected_vals = $('.leadout-selected-contacts');
...
jQuery(document).ready( function ( $ ) {
	$('#filter_action, #filter_content, #filter_form').change(function() {
		$('#leadout-contacts-filter-button').addClass('button-primary');
	});
});